...
# --- 5. AUTHENTICATED USER INTERFACE ---
  ...
  # Calculate total unread private messages safely for the badge
  ...
  if current_tab == "Directory":
    st.markdown(
        f"""
        <div style="background: linear-gradient(135deg, #0d9488 0%, #14b8a6 100%); padding: 20px; border-radius: 12px; color: white; margin-bottom: 20px;">
  ...
  # --- MERGED COMMUNICATION & FEED HUB TAB ---
  elif current_tab == "CommHub":
    st.markdown(
        f"""
        <div style="background: linear-gradient(135deg, #6366f1 0%, #ec4899 100%); padding: 20px; border-radius: 12px; color: white; margin-bottom: 20px;">
            <h1 style="margin:0; font-weight:700;">💬 Communication & Feed Hub — {user_org}</h1>
            <p style="margin:5px 0 0 0; opacity: 0.9;">Official Notices, Community Discussions & 1-on-1 Private Messages</p>
        </div>
        """,
        unsafe_allow_html=True,
    )

    # Sub-tabs for the merged communication page
    sub_tab_choice = st.radio(
        "Select Communication View",
        ["📢 Official Notice Board", "💬 Community Discussion Feed", "🔒 Secure Private Messages"],
        horizontal=True,
    ...
    # 1. NOTICE BOARD SUB-TAB
    if sub_tab_choice == "📢 Official Notice Board":
      ...
      if org_notices.empty:
        st.info("No official notices have been published for your organization yet.")
      else:
    ...
    # 2. COMMUNITY FEED SUB-TAB
    elif sub_tab_choice == "💬 Community Discussion Feed":
      ...
      if org_posts.empty:
        st.info("No community posts yet. Be the first to share an update with your team!")
      else:
    ...
    # 3. PRIVATE MESSAGES SUB-TAB
    elif sub_tab_choice == "🔒 Secure Private Messages":
      col_p1, col_p2 = st.columns([4, 1])
      with col_p1:
        st.markdown("### 1-on-1 Private Chat & Media Exchange")
      with col_p2:
